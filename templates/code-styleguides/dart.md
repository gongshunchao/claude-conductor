...
- **AVOID** using `cast()` when a nearby operation will do.
...
- **AVOID** defining a class that contains only static members; prefer top-level functions/variables.
- **AVOID** extending a class that isn't intended to be subclassed.
- **DO** use class modifiers (e.g., `final`, `interface`, `sealed`) to control subclassing.
- **PREFER** defining a pure mixin or pure class to a `mixin class`.

### 4.4. Constructors

- **CONSIDER** making your constructor `const` if the class supports it (all fields are `final`).
...
- **AVOID** returning nullable `Future`, `Stream`, and collection types; prefer empty containers.
