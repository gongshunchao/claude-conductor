...
- **Include Order:**
  ...
  2. C system (`<unistd.h>`)
  3. C++ standard (`<vector>`)
  4. Other libraries (`<absl/strings/str_cat.h>`)
...
- **Concepts:** Prefer C++20 Concepts (`requires`) over `std::enable_if`. Use `requires(Concept)`, not `template<Concept T>`.
...
- **Aliases:** Use `using` instead of `typedef`. Public aliases must be documented. Don't use in public API for convenience.
